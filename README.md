...
- Manual CashApp, PayPal, and Venmo review
...
Stars payment history, last purchase, access keys, credits, age confirmation, lightweight room memory, and manual payment review requests are stored locally in `data/users.json`.
...
The direct payment doors are centralized in `src/paymentDoors.ts`. They appear as additional doors in girlfriend, private, entry, spoil, worship, and reup flows. They do not auto-unlock access. Users can send a manual review request with a note or screenshot, and admins receive the submitted proof with request-specific review buttons.
...
For payment questions, users can send `/paysupport`. It shows their Telegram ID, current credits/access, last purchase, pending manual review status, and clear guidance for Stars and direct payments.
...
Card checkout and card webhooks are intentionally not part of this bot. Automatic access uses Telegram Stars only.
